...
def pipe(path, radius, segments=16, caps=True, subdiv=1, bend=None):
    ...
    ring in the bisecting plane at the pipe's own radius -- a pinch, down to
    31 % of the bore on the intake runners' 144-degree turns -- and where the
    turn is tighter than the pipe is fat the rings either side cross it and
    the tube folds through itself, which 22 pipes on this engine did. A pipe
    that is meant to have a sharp corner passes bend=0.
